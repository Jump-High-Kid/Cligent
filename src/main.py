...
# dashboard router 는 main.py 하단(공지 admin 페이지 라우트 다음)에서 등록.
# /announcements/new · /announcements/{ann_id}/edit 가 dashboard 의
...
#    /api/settings/plan/usage, /api/blog/beta-usage


# /api/feedback + _persist_feedback / _write_feedback_report / _FEEDBACK_BATCH
# → routers/dashboard.py 로 이동 (v0.9.0 / 6/6).
...
# /api/blog-chat/turn, /generate, /generate-image-prompts, _stream_and_save
# → routers/blog.py 로 이동 (v0.9.0 C2 / 2026-05-02).
...
# /announcements (목록·상세 페이지)·/api/announcements (read GET·POST read)
# → routers/dashboard.py 로 이동 (v0.9.0 / 6/6).
# /announcements/new·/announcements/{ann_id}/edit·작성/수정/삭제·upload-image
# 는 admin.py 로 이동 예정 (라우트 등록 순서 — dashboard 가 main.py 끝에서
# include 되도록 main.py 하단에서 include_router 호출).
...
# POST /api/announcements/{ann_id}/read → routers/dashboard.py (v0.9.0 / 6/6).
...
# ── dashboard router 등록 (라우트 등록 순서 보존) ──────────────────
# /announcements/new (admin) 가 /announcements/{ann_id} (dashboard) 보다
# 먼저 등록되도록 main.py 의 모든 inline 라우트 다음에 include.
